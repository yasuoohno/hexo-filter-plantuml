...
function localSideRendering(config, str) {
    ...
    const args = function (txtFile) {
        var command_line = [
            ...
            // avoid encoding default differences in Java Runtime.
            '-charset', charset
        ];

        command_line.push('-t' + outputFormat, '-graphvizdot', GraphvizDotFile);
        if (isDarkmode) command_line.push('-darkmode');
        command_line.push(txtFile);
